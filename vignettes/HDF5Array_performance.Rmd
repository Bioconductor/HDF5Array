...
date: "Compiled `r doc_date()`; Modified 13 January 2024"
package: HDF5Array
vignette: |
...
# Code for normalization and PCA
...
- The block size is 100 Mb by default.
...
command `top`) remained < 3.7 Gb at all time.
...
command `top`) remained < 2.8 Gb at all time.
...
With this block size (100 Mb), memory usage (as reported by Unix
command `top`) remained < 2.4 Gb at all time.
...
With this block size (100 Mb), memory usage (as reported by Unix
command `top`) remained < 2.7 Gb at all time.
